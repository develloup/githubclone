...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      const response = await fetch("/api/loggedinuser", { credentials: "include" }); // ✅ Korrigierte API-Route
    ...
    fetchUser();
  }, [router]); // ✅ `router` als Dependency enthalten
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      <div className="p-6 bg-white shadow-md rounded-lg">
        ...
        <button
          onClick={() => {
            document.cookie = "session_id=; Max-Age=0"; // Löscht das Session-Cookie
            router.push("/login");
          }}
