...
export default function BranchesPage() {
    ...
    const branchTable = branches?.all?.data?.repository?.refs?.nodes ?? [];
    const pageInfo = branches?.all?.data?.repository?.refs?.pageInfo;
    ...
    const mergedTable = defaultBranch?.length ? [defaultBranch[0], ...branchTable] : branchTable;
    ...
    return (
        <div className="max-w-[1340px] mx-auto px-4 py-6 space-y-6 mt-12">
            ...
            {/* All */}
            <section>
                <h2 className="text-sm font-semibold mt-6 mb-2">All branches</h2>
                <BranchTable branches={mergedTable} pageInfo={pageInfo} currentPage={page} currentPath={currentPath}/>
            </section>
        </div>
    );
}
